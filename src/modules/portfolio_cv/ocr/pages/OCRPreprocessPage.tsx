//src/modules/portfolio_cv/ocr/pages/OCRPreprocessPage.tsx
...
type PreprocessingType = "auto" | "grayscale" | "binarize" | "denoise" | "deskew" | "enhance";
...
const PREPROCESSING_OPTIONS: {
...
}[] = [
  {
    value: "auto",
    label: "Automático",
    description: "Detecta y aplica el mejor preprocesamiento",
  },
  {
    value: "enhance",
    label: "Mejora Avanzada",
    description: "Ideal para texto en fondos complejos",
  },
  {
    value: "grayscale",
    label: "Escala de Grises",
    description: "Convierte a blanco y negro",
  },
  {
    value: "binarize",
    label: "Binarización",
    description: "Texto blanco sobre fondo negro",
  },
  {
    value: "denoise",
    label: "Eliminar Ruido",
    description: "Limpia la imagen de artefactos",
  },
  {
    value: "deskew",
    label: "Corregir Inclinación",
    description: "Endereza documentos torcidos",
  },
];

const OCRPreprocessPage = () => {
  ...
  const [preprocessing, setPreprocessing] =
    useState<PreprocessingType>("auto");
  const [result, setResult] = useState<OCRPreprocessResponse | null>(null);
  ...
  const handleDetect = async () => {
    ...
    try {
      const response = await ocrService.preprocess({
    ...
    <Container fluid className="py-4">
        ...
        <div>
          <nav aria-label="breadcrumb">
            <ol className="breadcrumb mb-1">
              ...
              <li className="breadcrumb-item active">Preprocesamiento</li>
            </ol>
          </nav>
          <h2 className="mb-1">
          ...
          <p className="text-muted mb-0">
            Mejora la calidad de imagen antes de extraer texto
          </p>
        </div>
        <Button
        ...
        <Col lg={5} xl={4}>
          <Card className="border-0 shadow-sm mb-3">
            ...
            <Card.Body>
              ...
              {previewUrl && (
                <div className="mb-3">
                  <Form.Label>Vista previa original</Form.Label>
                  <div
              ...
              <Form.Group className="mb-3">
                <Form.Label>Tipo de preprocesamiento</Form.Label>
                <Form.Select
                  value={preprocessing}
                  onChange={(e) =>
                    setPreprocessing(e.target.value as PreprocessingType)
                  }
                >
          ...
          {!result && !error && (
            <Card className="border-0 shadow-sm">
              <Card.Body className="text-center py-5">
                <i className="fas fa-wand-magic-sparkles fa-4x text-muted mb-3 opacity-25"></i>
                <h5 className="text-muted mb-2">Sin resultados aún</h5>
                <p className="text-muted">
                  Selecciona una imagen y tipo de preprocesamiento
                </p>
              </Card.Body>
            </Card>
          )}
      ...
      >
        ...
        <Modal.Body className="p-4">
          ...
          <p className="text-muted">
            Aplica técnicas de <strong>preprocesamiento de imágenes</strong>{" "}
            usando OpenCV para mejorar la calidad antes de extraer texto con
            Tesseract OCR. Esto aumenta significativamente la precisión.
          </p>
          <h6 className="mt-3">Técnicas disponibles</h6>
          <ul>
            <li>
              <strong>Escala de Grises:</strong> Convierte la imagen a blanco y
              negro
            </li>
            <li>
              <strong>Binarización:</strong> Separa texto del fondo usando
              umbral adaptativo
            </li>
            <li>
              <strong>Eliminación de Ruido:</strong> Limpia artefactos y
              imperfecciones
            </li>
            <li>
              <strong>Corrección de Inclinación:</strong> Endereza documentos
              escaneados torcidos
            </li>
            <li>
              <strong>Automático:</strong> Aplica todas las técnicas en
              secuencia
            </li>
          </ul>
        </Modal.Body>
...
export default OCRPreprocessPage;